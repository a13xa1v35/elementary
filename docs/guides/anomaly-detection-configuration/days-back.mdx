...
- **Full time buckets** - Elementary will increase the `days_back` automatically to ensure full time buckets. For example if the `time_bucket` of the test is `period: week`, and 14 `days_back` result in Tuesday, the test will collect 2 more days back to complete a week (starting on Sunday).  
